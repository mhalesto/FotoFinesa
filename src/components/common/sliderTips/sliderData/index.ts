import {ISliderTypes} from './sliderTypes';

const sliderData: ISliderTypes[] = [
  {
    id: 1,
    title: 'Foto Finesa Premium',
    description: 'Unlock premium features & content',
    url: 'https://cdn.pixabay.com/photo/2017/06/17/10/55/hot-air-balloon-2411851_1280.jpg',
    actionButtonText: 'Start FREE Trial',
    // leftIcon: 'star',
    // leftIconColor: '#FFD700',
    // leftIconColor: '#6336D3',
    // sliderTextContainerStyle: {backgroundColor: 'rgba(0, 0, 0, 0.150)'},
    // sliderTextContainerStyle: {backgroundColor: 'rgba(0, 0, 0, 0.200)'},
  },
  {
    id: 2,
    title: 'Neon Lights',
    description: 'Unlock premium features',
    url: 'https://cdn.pixabay.com/photo/2021/04/20/16/32/woman-6194370_1280.jpg',
    leftIcon: 'weather-sunset-down',
    actionButtonText: 'Try Now',
    // url: 'https://cdn.pixabay.com/photo/2018/01/28/13/24/portrait-3113651_1280.jpg',
  },
  {
    id: 3,
    title: 'Overlays',
    description: 'Bring more layers to your photos.',
    // url: 'https://cdn.pixabay.com/photo/2017/12/28/16/18/bicycle-3045580_1280.jpg',
    // url: 'https://cdn.pixabay.com/photo/2015/05/11/14/51/heart-762564_1280.jpg',
    url: 'https://cdn.pixabay.com/photo/2016/02/13/10/35/tulips-1197602_1280.jpg',
    // url: 'https://cdn.pixabay.com/photo/2014/12/08/02/59/benches-560435_1280.jpg',
    // url: 'https://cdn.pixabay.com/photo/2018/03/10/17/16/woman-3214594_1280.jpg',
    actionButtonText: 'Try Now',
    sliderTextContainerStyle: {backgroundColor: 'rgba(0, 0, 0, 0.500)'},
    // sliderTextContainerStyle: {backgroundColor: 'rgba(0, 0, 0, 0.750)'},
    // leftIcon: 'air-filter',
  },
];

export default sliderData;
